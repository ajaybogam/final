    ...
    <Box>
      <PageWrapper className="" my={16}>
        <Box py={8}>
          ...
          <Text maxW="480px" mt="16px" fontSize="xl"> Last updated on 24 Jan 2020</Text>
        </Box>
        <Flex flexDirection={{ base: "column", md: "row" }}>
          <Box w={{ base: "100%", md: "30%" }} py={1}>
            <Box boxShadow="0 1px 8px rgba(0,0,0,.2)" px={4} overflow="hidden">
              {headings.map((heading, idx) => <Text
                display="block"
          ...
          <Box ref={sectionRef} w={{ base: "100%", md: "70%" }} p={8} mt={{ base: 16, md: 2 }} height="560px" overflow="hidden" overflowY="auto">
            {terms.map((term, idx) => <Box
              key={idx}
              id={`section-${idx}`}
              p={{ base: 2, md: 8 }}
              pt={0}
              color="gray.600">
              <Text fontWeight="bold" fontSize="xl" mb={idx === 0 ? 0 : 4}>{term.section}</Text>
              <Stack spacing={4}>
                {term.heading.map((content, index) => <Text key={index}>{content}</Text>)}
              </Stack>
