...
function Partners() {
  return (
    <Box>
      <PageWrapper>
        <MainTitle title="our financial partners" color="blue.400" textAlign="center" pt={8} />
        <SimpleGrid display={{base: "none", md: "grid"}} columns={{ base: 1, md: 3 }} >
          {partnerList.map((partner, idx) => <PartnerImage key={idx} as="img" mx="auto" my={{ base: 2, md: 4 }} src={partner} />)}
        </SimpleGrid>
  ...
  return <Box {...props}>
    <Carousel>
      {rowsIndexes.map((row, key) => <SimpleGrid key={key} columns={1}>
        {row.map((partnerIdx, idx) => <PartnerImage key={idx} as="img" mx="auto" my={{ base: 2, md: 4 }} src={partnerList[partnerIdx]} />)}
      </SimpleGrid>)}
    </Carousel>
  </Box>
}
